...
本次 POC 专注于验证**支付通知机制**,具体包括:

1. **Coinbase Commerce 集成**: 创建支付请求,生成支付链接
2. **零 Gas 费支付**: 用户使用任意钱包完成支付,无需支付 Gas 费
3. **Webhook 接收**: Auth 服务器接收 Coinbase Commerce 的支付通知
4. **签名验证**: 验证 Webhook 请求的真实性
5. **数据解析**: 从 Webhook 中提取用户地址、金额、时间、交易哈希
6. **数据持久化**: 将支付记录保存到 `payments.json` 文件

**验证边界**: 
- 验证到 Auth 服务器成功记录支付信息为止
- 后续的订阅管理、Xray 用户添加等业务逻辑不在本次验证范围内
