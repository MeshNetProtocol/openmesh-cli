...
try {
    ...
    if ($null -eq $first) { throw "stream closed before first event" }
    ...
    if (-not $start.ok) { throw "start_vpn failed during stream test: $($start.message)" }
    ...
    while ($true) {
        ...
        if ($evt.streamType -eq "heartbeat") {
            $sawHeartbeat = $true
        }
    }

    if ($events.Count -lt 3) { throw "expected at least 3 stream events, got: $($events.Count)" }
    if (-not $sawVpnOnDelta) { throw "expected to observe a delta event with vpnRunning=true" }
    if (-not $sawHeartbeat) { throw "expected to observe at least one heartbeat event" }

    $stop = Invoke-Core @{ action = "stop_vpn" }
    if (-not $stop.ok) { throw "stop_vpn failed after stream test: $($stop.message)" }

    Write-Host "P4 go core status stream checks passed."
}
finally {
    ...
    if ($null -ne $streamPipe) {
        try { $streamPipe.Dispose() } catch {}
    }
