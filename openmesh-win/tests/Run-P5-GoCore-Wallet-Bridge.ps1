...
try {
    ...
    if ([bool]$realPayResp.ok) {
        Assert-True -condition ((([string]$realPayResp.message).ToLowerInvariant().Contains("(real)"))) -message "strict real mode returned ok but response is not marked real mode"
        ...
        Assert-True -condition ((([string]$realPayResp.message).ToLowerInvariant().Contains("real"))) -message "strict real mode failed but message does not mention real-mode failure"
    }
