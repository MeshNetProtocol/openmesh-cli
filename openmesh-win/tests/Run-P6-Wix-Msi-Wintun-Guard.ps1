...
$outputDir = Join-Path $env:TEMP ("openmesh-win-p6r9-msi-output-" + [Guid]::NewGuid().ToString("N"))
$expandDir = Join-Path $env:TEMP ("openmesh-win-p6r9-msi-expand-" + [Guid]::NewGuid().ToString("N"))
$fakeWintun = Join-Path $env:TEMP ("openmesh-win-p6r9-fake-wintun-" + [Guid]::NewGuid().ToString("N") + ".dll")
$missingWintun = Join-Path $env:TEMP ("openmesh-win-p6r9-missing-wintun-" + [Guid]::NewGuid().ToString("N") + ".dll")
$msiVersion = "0.1.0-p6r9"
...
$case1 = Invoke-ScriptCapture -filePath $buildMsiScript -argumentsList @(
    "-Configuration", $Configuration,
    "-Version", "0.1.0-p6r9-missing",
    "-OutputDir", $outputDir,
    "-RequireWintun",
...
if ($case1.ExitCode -ne 0 -and (($case1.Output -join "`n") -match "Configured wintun source path not found")) {
    Add-Result "PASS" "require_missing_wintun" "Missing explicit wintun path is rejected in MSI pipeline."
} else {
    Add-Result "FAIL" "require_missing_wintun" ("Expected missing wintun rejection. exit=" + $case1.ExitCode + "; output=" + (($case1.Output | Select-Object -Last 8) -join " | "))
}

$case2 = Invoke-ScriptCapture -filePath $buildMsiScript -argumentsList @(
    "-Configuration", $Configuration,
    "-Version", $msiVersion,
    "-OutputDir", $outputDir,
    "-RequireWintun",
...
if ($case2.ExitCode -eq 0) {
    Add-Result "PASS" "msi_build_with_wintun" "MSI build succeeded with explicit wintun source."
} else {
    Add-Result "FAIL" "msi_build_with_wintun" ("MSI build failed. exit=" + $case2.ExitCode + "; output=" + (($case2.Output | Select-Object -Last 8) -join " | "))
}

$msiPath = Join-Path $outputDir ("OpenMeshWin-" + $msiVersion + ".msi")
if (Test-Path $msiPath) {
    Add-Result "PASS" "msi_artifact" ("MSI artifact present: " + $msiPath)
} else {
    Add-Result "FAIL" "msi_artifact" ("MSI artifact missing: " + $msiPath)
}
...
$reportPath = Join-Path $reportsDir ("p6-wix-msi-wintun-guard-" + $timestamp + ".txt")
$lines = New-Object System.Collections.Generic.List[string]
$lines.Add("OpenMeshWin P6 WiX MSI Wintun Guard")
...
Write-Host ("P6 wix msi wintun guard report written: " + $reportPath)
Write-Host ("Summary: FAIL=" + $failCount + " WARN=" + $warnCount)

if ($failCount -gt 0) {
    throw "P6 wix msi wintun guard checks failed."
}

Write-Host "P6 wix msi wintun guard checks passed."
