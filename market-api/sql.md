...
# 1) 先建表
npx wrangler d1 execute openmesh-market --local --file=./d1/001_schema.sql

# 2) 再导入官方在线 provider
npx wrangler d1 execute openmesh-market --local --file=./d1/002_seed_official_online.sql

npx wrangler d1 execute openmesh-market --local --command="SELECT name, sql FROM sqlite_master WHERE type='table' ORDER BY name;"
