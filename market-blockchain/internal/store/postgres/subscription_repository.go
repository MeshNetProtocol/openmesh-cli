...
func (r *SubscriptionRepository) Create(sub *domain.Subscription) error {
	query := `
		INSERT INTO subscriptions (
			...
			current_period_start, current_period_end, next_plan_id, last_charge_id,
			last_charge_at, source, created_at, updated_at
	...
	_, err := r.store.DB.Exec(query,
		...
		sub.LastChargeID, sub.LastChargeAt, sub.Source, sub.CreatedAt, sub.UpdatedAt,
	)
	return err
}
