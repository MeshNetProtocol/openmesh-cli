...
//  MeshFluxMac
//
//  连接 extension 的 command.sock 获取实时连接列表（与 sing-box ConnectionListView / CommandClient(.connections) 一致）。
//
...
/// 连接 extension 的 CommandClient(.connections)，接收连接列表；支持筛选、排序、关闭连接。
public final class ConnectionCommandClient: ObservableObject {
    ...
    /// 按当前筛选/排序刷新列表（rawConnections 已由 extension 更新时调用）
