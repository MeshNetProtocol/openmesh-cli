...
4. **x402 是热路径，小额自动支付是核心能力。**  
   供应商可以先免费提供一个流量桶；在剩余流量低于阈值时发起 `402`，客户端自动签名支付下一桶费用。
...
5. **用户和供应商都保存彼此签名的凭条，任何一方丢失都可向对方恢复。**
6. **流量统计和热路径放行不依赖平台中心化服务器。**
7. **热路径放行不等待链上余额轮询，应以 x402 校验通过为准；链上监听只做异步对账。**
