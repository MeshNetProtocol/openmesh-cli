...
#### 模块 M9 细分执行（进一步小步化）
- 为避免一次性联调过大，M9 拆分为 4 个可独立测试子模块，每个子模块通过后再进入下一个：
1. `M9.1` 在线市场闭环回归：只测 Marketplace 页面的 Install/Update/Reinstall 与安装向导状态展示。
2. `M9.2` 已安装管理闭环回归：只测 Installed 页面列表正确性、Update/Reinstall、Uninstall 阻断与成功路径。
3. `M9.3` 离线导入闭环回归：只测 URL/文件/文本导入、遮罩防重入、3 次重试、WebView 回退日志。
4. `M9.4` 平权与隔离约束回归：只测“无默认官方供应商注入”“供应商目录互不影响”“映射精确清理”。

- 每个子模块退出标准固定为：
1. `MeshFluxIos` 编译通过。
2. 仅本子模块用例执行通过（避免交叉污染）。
3. 记录一条简短验收结论（通过/失败 + 失败点）。

#### 当前进度（持续更新）
1. `M1` 已完成（Tab 结构重构）。
2. `M2` 已完成（去默认供应商自动注入）。
3. `M3` 已完成（MarketService 核心接入）。
4. `M4` 已完成（Market 入口页）。
5. `M5` 已完成（Marketplace + 安装向导）。
6. `M6` 已完成（Installed 独立页）。
7. `M7` 已完成（卸载向导闭环）。
8. `M8` 已完成（离线导入增强：遮罩/重试/诊断/WebView 回退）。
9. `M9` 进行中（按 `M9.1 -> M9.4` 小步执行）。
