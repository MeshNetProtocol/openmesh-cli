...
目前菜单栏宽窗口已具备可用 UI 结构（含 DashBoard/Market/Settings 3 个 Tab、流量图表、节点切换窗口、左下角齿轮菜单）。  
但以下核心能力仍处于 **UI 已具备/数据是假或未接入** 的状态：

- 流量图表（上下行曲线）与“节点+上下行速率”行：数据来自假数据定时器（未接 `StatusCommandClient`）。
- 节点切换窗口：节点列表/延迟/测速为假数据（未接 `GroupCommandClient` / extension 命令）。
- “未连接 VPN 时也能展示并测速出站组（节点）”：当前代码体系下没有现成实现，需要新增机制（见第 6 节方案）。
...
2. DashBoard Tab：VPN 控制 + 流量商户下拉 + 流量曲线图 + 节点+速率行（当前数据为假）
3. More info：单页面窗口（假数据）
4. 节点切换：单页面窗口（假数据，含全局/单行“测速”按钮）
...
|---|---|---|---|---|
| 流量曲线图（上下行） | Dashboard/ExtensionStatus 的流量展示（部分） | `openmesh-apple/MeshFluxMac/views/DashboardView.swift`（`StatusCommandClient` + `OMLibboxStatusMessage`） | UI 有，数据假 | 接入 `StatusCommandClient.status`，把 `uplink/downlink`、累计等映射到图表与数字 |
| “节点+速率”信息行 | 旧「出站组」+ 旧「流量」的组合 | 旧「出站组」：`GroupCommandClient`；旧「流量」：`StatusCommandClient` | UI 有，数据假 | 连接后用 `GroupCommandClient.groups` 得到选中节点 + 延迟；用 `StatusCommandClient` 得到实时速率 |
| “切换”按钮 → 节点窗口 | 旧「出站组」页面（节点选择/测速） | `openmesh-apple/MeshFluxMac/views/GroupsView.swift` + `GroupCommandClient` | UI 有，数据假 | 用真实 `groups` 生成节点列表；实现 urlTest / selectOutbound；维护 selection 状态 |
...
1. 在 DashBoard Tab 中引入 `StatusCommandClient`（或复用 DashboardView 的实现），获取 `OMLibboxStatusMessage`：
   - 上下行速率：`uplink/downlink`
   - 累计：`uplinkTotal/downlinkTotal`（若可用）
2. 在 DashBoard Tab 中引入 `GroupCommandClient`：
   - 连接策略：仅当 `vpnController.isConnected` 时 connect；断开时 disconnect
   - 用 `groups` 找到主 selector/urltest 组的 `selected` 与 `items`，映射到“当前节点名/延迟”
3. 节点窗口内：
   - “测速”→ `GroupCommandClient.urlTest(groupTag:)`
   - 选中节点 → `GroupCommandClient.selectOutbound(groupTag:, outboundTag:)`

收益：VPN 连接后 UI 与旧「出站组」能力对齐；风险低，完全复用现有实现。
